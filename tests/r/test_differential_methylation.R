...
result <- system2("Rscript", c(
  ...
  "--contrast", "group547X-groupUTC",
  "--fdr", "0.05",
  "--min-abs-delta-beta", "0.05"
), stdout = TRUE, stderr = TRUE)
if (!identical(attr(result, "status"), NULL)) stop(paste(result, collapse = "\n"))

dmp <- read.delim(file.path(output_path, "complete", "group547X-groupUTC.tsv"), check.names = FALSE)
required <- c("delta_beta_adjusted", "delta_beta_observed", "mean_beta_547X", "mean_beta_UTC")
...
expected <- rowMeans(beta[dmp$probe_id, groups == "547X", drop = FALSE]) - rowMeans(beta[dmp$probe_id, groups == "UTC", drop = FALSE])
...
cat("DMP adjusted/observed delta-beta test passed for a numeric-leading group label.\n")
