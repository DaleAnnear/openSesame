...
group_summary <- function(result, contrast_name) {
  ...
  for (term in terms) {
    label <- labels[[term]]; ids <- ss$group == label; suffix <- gsub("[^A-Za-z0-9._-]", "_", label)
...
for (ct in colnames(cm)) {
  ...
  testable <- is.finite(fit$t[, ct]) & is.finite(fit$p.value[, ct]) & is.finite(fit$adj.p.value[, ct]) & is.finite(beta_fit$coefficients[, ct])
